...
@router.get(
    ...
    responses={
        404: {
            "description": "指定した条件のスナップショットが見つからない場合に返されます。",
        },
        500: {
            "description": "スナップショット情報の一覧を取得中にエラーが発生した場合に返されます。",
        },
    },
)
...
async def get_vm_snapshots(
    ...
    try:
        ...
        if isinstance(snapshots, list) and len(snapshots) > 0:
            ...
            raise HTTPException(
                status_code=404,
                detail=f"指定されたインスタンスUUID({vm_instance_uuid})の仮想マシンにスナップショットは存在しませんでした。",
            )
    except Exception as e:
